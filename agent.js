/**
 * GyaanSarita - Advanced AI Assistant
 * v2.8.0 (Updated: AI Pipe integration + bug fixes)
 * Author: Shreshta Sutar (Original) & Assistant (fixes)
 */
class GyaanSarita {
    constructor() {
        this.version = '2.8.0';
    ...
    async init() {
        try {
            ...
            console.log(`GyaanSarita v${this.version} initialized successfully`);
        } catch (error) {
            console.error('Failed to initialize GyaanSarita:', error);
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'GyaanSarita', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to GyaanSarita! How can I assist you?');
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.agentFlow = new GyaanSarita();
});
